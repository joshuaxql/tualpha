...
- 新增安全的 `$field` 因子表达式、129 个基础与技术算子以及回调 `data.history()` 集成；
- 新增离线 `factor_data()`，支持固定资产池或严格 PIT 指数成分，并按日过滤 ST、上市天数和停牌；
- 新增完整 Alphalens 风格单因子报告，展示因子公式、收益/IC/RankIC、分位、换手、稳定性、月度热力图和行业分析，并支持任意预测周期数组及 PIT 行业与市值中性化；
- 新增 2017-01-01 至 2026-01-01 的中证 1000 全算子性能基准，采用共享列预取和分批计算控制峰值内存。
- 合并因子、行业、市值和未来收益标签的共享字段扫描，并向量化 IC、分位收益和分行业统计；完整复杂因子报告基准总耗时降低约 55%。
