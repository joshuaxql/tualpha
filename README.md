...
公告日当天不可见，从公告日后的首个交易日开始可见。同一报告期选择当时可见的最新公告或修订。大型选股横截面可使用 `data.fundamental_arrays(assets, fields)` 批量读取最新可见财务值，返回数组顺序与输入资产一致。

### 指数日线与成分 PIT

指数不可交易，但策略可以读取配置白名单内的原始指数日线：
...
指数日线始终为原始点位，不受 `adjustment` 影响。指数成分使用：
...
- 仅支持日频回测；
- 仅支持多头现金账户；
- 股票和 ETF 可交易；指数不可交易，但可读取日线、作为基准或 PIT 成分来源；
